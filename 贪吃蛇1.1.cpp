...
void Snake::Rungame(){
	...
	while(1){
		timeleft=1;
        start=clock();
        while((timeleft=(clock()-start<=speed))&&!kbhit());        //帧与帧之间的时间控制 
        if(timeleft)
        {
            getch();
            direction = getch();
        }
		switch(direction){                                         //计算下一帧的蛇头坐标 
		...
		if(head_x==0||head_x==20||head_y==0||head_y==20){          //如果撞墙则死亡 
			cout<<"GAMEOVER!"<<endl;
			break;
		}
		if(m[head_x][head_y]!=' '&&m[head_x][head_y]!='$'){         //如果撞身体则死亡 
			cout<<"GAMEOVER!"<<endl;
			break;
		}
		
		if(head_x==food_x&&head_y==food_y){                         //吃到食物，并计算下一帧的画面 
			Eat();
			Food();
		} 
		else Render(); //如果没有吃到食物，计算下一帧的画面 。 
		Print();       //刷新画面 
	}
}
int main(){
